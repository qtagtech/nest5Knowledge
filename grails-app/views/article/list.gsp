			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="hits" title="${message(code: 'article.hits.label', default: 'Hits')}" />
					
						<g:sortableColumn property="rating" title="${message(code: 'article.rating.label', default: 'Rating')}" />
				...
				<g:each in="${articleInstanceList}" status="i" var="articleInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${articleInstance.id}">${fieldValue(bean: articleInstance, field: "category")}</g:link></td>
					
						<td>${fieldValue(bean: articleInstance, field: "content")}</td>
					
						<td><g:formatDate date="${articleInstance.date}" /></td>
					
						<td>${fieldValue(bean: articleInstance, field: "hits")}</td>
					
						<td>${fieldValue(bean: articleInstance, field: "rating")}</td>
